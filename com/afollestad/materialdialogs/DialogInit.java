...
class DialogInit {
    ...
    @StyleRes
    static int getTheme(@NonNull Builder builder) {
        boolean darkTheme = DialogUtils.resolveBoolean(builder.context, C0226R.attr.md_dark_theme, builder.theme == Theme.DARK);
        builder.theme = darkTheme ? Theme.DARK : Theme.LIGHT;
        return darkTheme ? C0226R.style.MD_Dark : C0226R.style.MD_Light;
    }

    @LayoutRes
    static int getInflateLayout(Builder builder) {
        if (builder.customView != null) {
            return C0226R.layout.md_dialog_custom;
        }
        if ((builder.items == null || builder.items.size() <= 0) && builder.adapter == null) {
            if (builder.progress > -2) {
                return C0226R.layout.md_dialog_progress;
            }
            if (builder.indeterminateProgress) {
                if (builder.indeterminateIsHorizontalProgress) {
                    return C0226R.layout.md_dialog_progress_indeterminate_horizontal;
                }
                return C0226R.layout.md_dialog_progress_indeterminate;
            } else if (builder.inputCallback != null) {
                if (builder.checkBoxPrompt != null) {
                    return C0226R.layout.md_dialog_input_check;
                }
                return C0226R.layout.md_dialog_input;
            } else if (builder.checkBoxPrompt != null) {
                return C0226R.layout.md_dialog_basic_check;
            } else {
                return C0226R.layout.md_dialog_basic;
            }
        } else if (builder.checkBoxPrompt != null) {
            return C0226R.layout.md_dialog_list_check;
        } else {
            return C0226R.layout.md_dialog_list;
        }
    }
    ...
    public static void init(MaterialDialog dialog) {
        ...
        if (builder.backgroundColor == 0) {
            builder.backgroundColor = DialogUtils.resolveColor(builder.context, C0226R.attr.md_background_color, DialogUtils.resolveColor(dialog.getContext(), C0226R.attr.colorBackgroundFloating));
        }
        if (builder.backgroundColor != 0) {
            GradientDrawable drawable = new GradientDrawable();
            drawable.setCornerRadius(builder.context.getResources().getDimension(C0226R.dimen.md_bg_corner_radius));
            drawable.setColor(builder.backgroundColor);
            DialogUtils.setBackgroundCompat(dialog.view, drawable);
        }
        if (!builder.positiveColorSet) {
            builder.positiveColor = DialogUtils.resolveActionTextColorStateList(builder.context, C0226R.attr.md_positive_color, builder.positiveColor);
        }
        if (!builder.neutralColorSet) {
            builder.neutralColor = DialogUtils.resolveActionTextColorStateList(builder.context, C0226R.attr.md_neutral_color, builder.neutralColor);
        }
        if (!builder.negativeColorSet) {
            builder.negativeColor = DialogUtils.resolveActionTextColorStateList(builder.context, C0226R.attr.md_negative_color, builder.negativeColor);
        }
        if (!builder.widgetColorSet) {
            builder.widgetColor = DialogUtils.resolveColor(builder.context, C0226R.attr.md_widget_color, builder.widgetColor);
        }
        if (!builder.titleColorSet) {
            builder.titleColor = DialogUtils.resolveColor(builder.context, C0226R.attr.md_title_color, DialogUtils.resolveColor(dialog.getContext(), 16842806));
        }
        if (!builder.contentColorSet) {
            builder.contentColor = DialogUtils.resolveColor(builder.context, C0226R.attr.md_content_color, DialogUtils.resolveColor(dialog.getContext(), 16842808));
        }
        if (!builder.itemColorSet) {
            builder.itemColor = DialogUtils.resolveColor(builder.context, C0226R.attr.md_item_color, builder.contentColor);
        }
        dialog.title = (TextView) dialog.view.findViewById(C0226R.id.md_title);
        dialog.icon = (ImageView) dialog.view.findViewById(C0226R.id.md_icon);
        dialog.titleFrame = dialog.view.findViewById(C0226R.id.md_titleFrame);
        dialog.content = (TextView) dialog.view.findViewById(C0226R.id.md_content);
        dialog.recyclerView = (RecyclerView) dialog.view.findViewById(C0226R.id.md_contentRecyclerView);
        dialog.checkBoxPrompt = (CheckBox) dialog.view.findViewById(C0226R.id.md_promptCheckbox);
        dialog.positiveButton = (MDButton) dialog.view.findViewById(C0226R.id.md_buttonDefaultPositive);
        dialog.neutralButton = (MDButton) dialog.view.findViewById(C0226R.id.md_buttonDefaultNeutral);
        dialog.negativeButton = (MDButton) dialog.view.findViewById(C0226R.id.md_buttonDefaultNegative);
        ...
        if (builder.icon != null) {
            ...
            Drawable d = DialogUtils.resolveDrawable(builder.context, C0226R.attr.md_icon);
            if (d != null) {
                dialog.icon.setVisibility(0);
        ...
        if (maxIconSize == -1) {
            maxIconSize = DialogUtils.resolveDimension(builder.context, C0226R.attr.md_icon_max_size);
        }
        if (builder.limitIconToDefaultSize || DialogUtils.resolveBoolean(builder.context, C0226R.attr.md_icon_limit_icon_to_default_size)) {
            maxIconSize = builder.context.getResources().getDimensionPixelSize(C0226R.dimen.md_icon_max_size);
        }
        if (maxIconSize > -1) {
        ...
        if (!builder.dividerColorSet) {
            builder.dividerColor = DialogUtils.resolveColor(builder.context, C0226R.attr.md_divider_color, DialogUtils.resolveColor(dialog.getContext(), C0226R.attr.md_divider));
        }
        ...
        if (VERSION.SDK_INT >= 14) {
            ...
            if (textAllCaps) {
                textAllCaps = DialogUtils.resolveBoolean(builder.context, C0226R.attr.textAllCaps, true);
            }
        } else {
            textAllCaps = DialogUtils.resolveBoolean(builder.context, C0226R.attr.textAllCaps, true);
        }
        ...
        if (dialog.recyclerView != null) {
            if (builder.adapter == null) {
                ...
                builder.adapter = new DefaultRvAdapter(dialog, ListType.getLayoutForType(dialog.listType));
        ...
        if (builder.customView != null) {
            ((MDRootLayout) dialog.view.findViewById(C0226R.id.md_root)).noTitleNoPadding();
            FrameLayout frame = (FrameLayout) dialog.view.findViewById(C0226R.id.md_customViewFrame);
            ...
            if (builder.wrapCustomViewInScroll) {
                ...
                int framePadding = r.getDimensionPixelSize(C0226R.dimen.md_dialog_frame_margin);
                View scrollView = new ScrollView(dialog.getContext());
                int paddingTop = r.getDimensionPixelSize(C0226R.dimen.md_content_padding_top);
                int paddingBottom = r.getDimensionPixelSize(C0226R.dimen.md_content_padding_bottom);
        ...
        if (builder.indeterminateProgress || builder.progress > -2) {
            ...
            if (dialog.mProgress != null) {
                ...
                if (!builder.indeterminateProgress || builder.indeterminateIsHorizontalProgress) {
                    ...
                    dialog.mProgressLabel = (TextView) dialog.view.findViewById(C0226R.id.md_label);
                    ...
                    dialog.mProgressMinMax = (TextView) dialog.view.findViewById(C0226R.id.md_minMax);
    ...
    private static void setupInputDialog(MaterialDialog dialog) {
        ...
        if (dialog.input != null) {
            ...
            dialog.inputMinMax = (TextView) dialog.view.findViewById(C0226R.id.md_minMax);
